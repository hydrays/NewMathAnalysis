...
> ## 例1：饮马路径问题（均匀速度）
> 
> **问题：** 骑马从点A到点B，但必须先去河边（直线）喝一次水。已知点A和点B在河的同侧，求最短路径。
> 
> **建立坐标系：**
> - 设河边为x轴。
> - 点A坐标为(0, h₁)，点B坐标为(l, h₂)，其中h₁ > 0, h₂ > 0。
> - 设饮水点为P(x, 0)，其中x ∈ [0, l]。
> 
> ![](../media/img/chpt2_horse_drinking_problem.png)
> 
> ### 解法一：微积分法
> 
> 路径总长度S(x)为：
...
> 
> 引入角度关系：
> - 设入射角为θ₁，满足 $\sin \theta_1 = \frac{x}{\sqrt{h_1^2 + x^2}}$
> - 设反射角为θ₂，满足 $\sin \theta_2 = \frac{l - x}{\sqrt{h_2^2 + (l - x)^2}}$
> 
> 因此，有：
> $$
...
> $$
> 
> 由于θ₁和θ₂都在(0, π/2)范围内，所以θ₁ = θ₂。这意味着最短路径满足入射角等于反射角。
> 
> ### 解法二：镜面原理法
> 
> - 将点B关于x轴（河边）反射到点B'，坐标为(l, -h₂)。
> - 连接点A(0, h₁)和点B'(l, -h₂)，与x轴的交点即为饮水点P。
> - 路径A→P→B的长度等于A→P→B'的长度，而A到B'的直线段是最短路径，因此A→P→B是最短路径。
> 
> **证明：** 对于任意其他点P'，路径A→P'→B = A→P'→B' ≥ A→B'（三角形不等式），等号仅当P'在A和B'的连线上时成立。
> 
> **结论：** 在均匀速度下，最短路径满足入射角等于反射角，这可以通过镜面反射原理快速求解。
> 
> ![](../media/img/chpt2_horse_drinking_problem.png)
> 
> ## 例2：镜面反射问题（不同速度）
> 
> **问题：** 光线从点A(0, h₁)出发，经镜面（x轴）反射到达点B(l, h₂)。已知光在A侧介质中速度为v₁，在B侧介质中速度为v₂，且v₁ ≠ v₂，求耗时最短的路径。
> 
> **解：**
> 
> 设反射点为P(x, 0)，则总传播时间为：
...
> 
> **讨论：**
> - 当v₁ = v₂时，得到sin θ₁ = sin θ₂，即θ₁ = θ₂，这就是经典的镜面反射定律。
> - 当v₁ ≠ v₂时，反射角不等于入射角，这是对传统反射定律的推广。
> 
> 
> 
> ## 例3：折射问题（光路最短原理）
> 
> **问题：** 一束光线从空气中的点A出发，经过水面折射后到达水中的点B。已知光在空气中和水中的传播速度分别为v₁和v₂，试确定光线从点A到点B传播耗时最短的路径。
> 
> **解：**
> 
> 如前所述，通过微积分推导，总传播时间T(x)最小化后得到：
> $$
> \frac{\sin \theta_1}{v_1} = \frac{\sin \theta_2}{v_2}
> $$
> 
> 这就是斯涅耳定律（Snell's Law），它描述了光线在两种介质交界面上的折射行为。
> 
> **光路最短原理（费马原理）** 指出光在两点之间传播时，所走的路径是光程最短的路径。以上例子展示了如何利用微积分证明光学中的基本定律，体现了数学工具在物理学中的强大应用。
> 
> ==光路最短原理==
>
