...
> - 如果 $x$ 不是集合 $A$ 中的元素, 就说 $x$ **不属于** 集合 $A$, 记作 $x \notin A$.
> - 不包含任何元素的集合叫做 **空集** (emptyset), 记为 $\varnothing$.
...
> **补集**
> 
> 如果集合 $A$ 是集合 $B$ 的子集, 则 $B$ 中不属于 $A$ 的所有元素构成的集合称为 $A$ 相对于 $B$ 的 **补集** (complementary set), 记作 $C_BA$ 或 $A^C$, 即
> 
> $$
...
> [!Important: 映射的定义]
> 
> 映射 (map) 描述了从一个集合到另一个集合的某种对映关系. 设 $A$, $B$ 是两个集合, 如果存在一个对映关系 $f$, 使得集合 $A$ 中的每个元素都唯一的对映到 $B$ 中的一个元素, 则称 $f$ 为从 $A$ 到 $B$ 的映射, 记作
> $$
> f: A \rightarrow B
> $$
>
...
> 
> 从上面的例子可以感受到, 映射的作用在于把集合 $A$ 中的元素作为**输入信号**, 例如不同的人、GPS坐标、光的波长等, 经过某种操作, 转换成我们关心的输出, 如名字、位置和颜色. 从这个意义上理解, 映射就是把输入 $x$ 转化成 $f(x)$ 的一个机器.
> 
...
> [!important: 数列]
> 
> **数列**是一类特殊的函数, 它把自然数集 $\mathbb{N}$ 映射到实数集 $\mathbb{R}$ 中, 这个函数可以记作 $a(n)$, 不过更多的时候我们会把 $n$ 作为下标, 以 $a_n$ 表示数列的第 $n$ 项, 并以 $\{ a_n \}$ 表示整个数列.
> 
> **等差数列**
> 
...
> - 等比数列的**前 $n$ 项和**为
> $$
> S_{n}=a_{1}\frac{1-q^n }{1-q} \quad (q\ne 1)
> $$
...
> [!important: 复合函数]
> 
> 对于函数 $f$ 和 $g$, 我们可以构造一个**新的函数** $y = f[g(x)]$, 它把自变量 $x$ 通过函数 $g$ 映射到 $u = g(x)$, 再把 $u$ 视作自变量 (也称为**中间变量**) 通过函数 $f$ 映射到 $y = f[g(x)]$. 这个过程可以表示为:
> 
> $$
...
> **一尺之捶, 日取其半 (版本1)**
> 
> 站在棒子的角度, 这个过程可以用一个数列描述. $n$ 代表天数, $a_n$ 代表棒剩下的长度.
>
> $$
> \begin{align*}
> a_0 &= 1,  \\
...
> $$
> 
> 随着 $n$ 的增加 (**动**), $a_n$ 的值不断靠近0 (**静**), 直觉告诉我们当 $n$ 趋于无穷大时 $a_n$ 的极限是0.
>
> **一尺之捶, 日取其半 (版本2)**
> 
...
> $$
>
> 随着 $n$ 的增加 (**动**), $b_n$ 的值不断靠近1 (**静**), 直觉告诉我们当 $n$ 趋于无穷大时 $b_n$ 的极限是1.
> 
> **P21 例1**
> 
> $$
...
> $$
> 
> 随着 $n$ 的增加 (**动**), $a_n$ 的值不断靠近1 (**静**), 直觉告诉我们当 $n$ 趋于无穷大时 $a_n$ 的极限是1.
>
> **P22 例2**
> 
> $$
...
> ==函数极限的定义== (非常重要!!!)
> 
> 设函数 $f(x)$ 在点 $x_0$ 的某一去心邻域内有定义 ( 为了让 $x$ 能够动起来), $A$ 为一常数. 如果**对于任意给定的正数 $\varepsilon$**, **总存在正数 $\delta$**, 使得当 $0 < |x-x_0| < \delta$ 时有
> $$
> |f(x) - A| < \varepsilon,
> $$
>
...
::: {.exercise id="chpt1-ex-022"}
:::

::: {.exercise id="chpt1-ex-023"}
:::

::: {.exercise id="chpt1-ex-024"}
:::
...
> **数列极限的定义**
> 
> $\displaystyle \lim_{n\rightarrow \infty} a_n = A \iff  \forall \varepsilon > 0$, $\exists N$, s. t., $\ |a_n - A| < \varepsilon$ if $n > N$.
> 
> **函数极限的定义**
> 
> $\displaystyle \lim_{x\rightarrow x_0} f(x) = A \iff \forall \varepsilon > 0$, $\exists \delta$, s. t., $|f(x) - A| < \varepsilon$ if $0 < |x-x_0| < \delta$.
> 
...
> **数列趋于无穷大**
> 
> $\displaystyle \lim_{n\rightarrow \infty} a_n = \infty \iff  \forall M > 0$, $\exists N$, s. t., $|a_n| > M$ if $n > N$.
> 
>**函数趋于无穷大**
> 
> $\displaystyle \lim_{x\rightarrow x_0} f(x) = \infty \iff \forall M > 0$, $\exists \delta$, s. t., $|f(x)| > M$ if $0 < |x-x_0| < \delta$.
> 
...
> [!caution: 希尔伯特的旅店]
>
> 极限运算涉及到了**无限**_~Rd~_的概念, 对初学者来说这是一个之前从未踏足过的未知领域, 一些**有限世界中的直觉**将不再成立. **希尔伯特的旅店**是一个有趣的故事, 通过这个故事希望能让大家对**无限**_~Rd~_有一颗**敬畏之心**.
>
...
::: {.exercise id="chpt1-ex-004"}
:::

::: {.exercise id="chpt1-ex-005"}
:::

::: {.exercise id="chpt1-ex-006"}
:::

::: {.exercise id="chpt1-ex-007"}
:::

::: {.exercise id="chpt1-ex-008"}
:::
...
> [!tip: 重要极限一]
> 
> 还记得"割圆法"求圆面积的例子吗? 圆的内接正 $n$ 边形的面积当 $n \rightarrow \infty$ 的极限的计算最后就会落到极限 $\displaystyle \lim_{x\rightarrow 0} \frac{\sin x}{x}$ 的计算, 这个极限与圆周率 $\pi$ (**阿基米德常数**_~Rd~_)有着深刻的联系.
...
> [!warning]
> 
> 假设 $y=f[g(x)]$,  $\displaystyle \lim_{x\rightarrow x_0}g(x) = u_0$, $\displaystyle \lim_{u\rightarrow u_0}f(u) = A$, 则 $\displaystyle \lim_{x\rightarrow x_0}f[g(x)] = A$.
> 
> 这个结论请自行根据直觉理解, 证明略.
>

::: {.exercise id="chpt1-ex-009"}
:::

::: {.exercise id="chpt1-ex-010"}
:::

::: {.exercise id="chpt1-ex-011"}
:::

::: {.exercise id="chpt1-ex-012"}
:::


::: {.exercise id="chpt1-ex-013"}
:::
...
::: {.exercise id="chpt1-ex-014"}
:::

::: {.exercise id="chpt1-ex-015"}
:::
...
::: {.exercise id="chpt1-ex-016"}
:::

::: {.exercise id="chpt1-ex-017"}
:::
...
> - **左连续**(注意极限记号下方的 $-$ 号)
> 
> $\displaystyle \lim_{x\rightarrow x_0^-} f(x) = A \iff \forall \varepsilon > 0$, $\exists \delta>0$, s. t., $|f(x) - A| < \varepsilon$ if $-\delta < x-x_0 < 0$.
> 
> - **右连续**(注意极限记号下方的 $+$ 号)
>
> $\displaystyle \lim_{x\rightarrow x_0^+} f(x) = A \iff \forall \varepsilon > 0$, $\exists \delta>0$, s. t., $|f(x) - A| < \varepsilon$ if $0 < x-x_0 < \delta$.
>
> 造成函数 $f(x)$ 在 $x=x_0$ 处的极限不存在的一个常见原因是 $f(x)$ 的**左右极限不相等**.



> [!important: 符号函数 $\mathrm{sgn}(x)$]
> 
> $$
...
::: {.exercise id="chpt1-ex-018"}
:::

::: {.exercise id="chpt1-ex-019"}
:::
...
::: {.exercise id="chpt1-ex-020"}
:::

> [!caution]
>
> 布尔查诺定理是一个**存在性定理**, 它只告诉我们一个东西有还是没有, 与之相对的是所谓的**构造性定理**, 后者会给出找出这个东西的具体办法. **存在性定理**和**构造性定理**都是数学的重要组成部分, 按照一般的理解, 前者在纯数学中的地位更高, 后者则一般更受工程师和程序员的青睐.
>
